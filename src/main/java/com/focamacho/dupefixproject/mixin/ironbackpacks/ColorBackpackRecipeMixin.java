...
public abstract class ColorBackpackRecipeMixin {
    ...
    private int getDyeColor(@Nonnull InventoryCrafting matrix) {
        ...
        return DyeUtils.colorFromStack(stack).map(dye -> colorValues.getOrDefault(dye.getUnlocalizedName(), BackpackInfo.NO_COLOR)).orElse(BackpackInfo.NO_COLOR); //Get dye RGB color and return it if possible, otherwise return no color
    }
