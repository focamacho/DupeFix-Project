package com.focamacho.dupefixproject.mixin.quark;
...
public abstract class EntityPickarangMixin extends EntityThrowable {

    @Shadow @Final private static ThreadLocal<Boolean> IS_PICKARANG_UPDATING;

    @Shadow @Final private static DataParameter<Boolean> RETURNING;

    @Shadow private int liveTime;

    @Shadow protected abstract void setReturning();

    @Shadow public abstract ItemStack getStack();

    @Shadow public abstract int getEfficiencyModifier();

    @Shadow private int slot;

    public EntityPickarangMixin(World worldIn) {
        super(worldIn);
    }
    ...
    public void onUpdate() {
        ...
        if(!returning) {
            ...
            Vec3d ownerPos = owner.getPositionVector().addVector(0, 1, 0);
