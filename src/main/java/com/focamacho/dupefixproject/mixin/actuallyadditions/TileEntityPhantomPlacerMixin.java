...
public abstract class TileEntityPhantomPlacerMixin extends TileEntityInventoryBase {
    ...
    @Shadow public boolean isBreaker;

    @Shadow public BlockPos boundPosition;

    @Shadow public int side;

    @Shadow public abstract void renderParticles();

    @Shadow private int oldRange;

    @Shadow public int range;

    @Shadow public int currentTime;

    @Shadow public abstract boolean hasBoundPosition();

    @Shadow @Final public static int RANGE;

    public TileEntityPhantomPlacerMixin(int slots, String name) {
        super(slots, name);
    }
